...
def build_candles(reference_price: float | None = None) -> list[Candle]:
    last = 78120.0
    candles: list[Candle] = []
    ...
    if reference_price and reference_price > 0:
        offset = reference_price - candles[-1].close
        candles = [
            Candle(
                time=candle.time,
...
def build_heat_bands(model: int, threshold: int = 0, reference_price: float | None = None) -> list[HeatBand]:
    ...
    for anchor_index, price in enumerate(anchors):
        ...
        for row in range(row_count):
            ...
            bands.append(
                HeatBand(
                    price=price - row * 72 + model * 18 + offset,
                    start=start,
    ...
    bands.extend(
        [
            HeatBand(price=79070 + offset, start=8, end=158, intensity=1.0),
            HeatBand(price=77490 + offset, start=8, end=244, intensity=0.97),
            HeatBand(price=76670 + offset, start=8, end=244, intensity=0.58 + model * 0.03),
            HeatBand(price=81380 + offset, start=8, end=244, intensity=0.48 + model * 0.03),
        ]
    )
...
def build_buckets(model: int, response_range: str, reference_price: float | None = None) -> list[HeatmapBucket]:
    offset = (reference_price - 78_350) if reference_price and reference_price > 0 else 0.0
    now = datetime.now(UTC)
    buckets: list[HeatmapBucket] = []
    for index in range(72):
        price_bucket = PRICE_MIN + (index / 71) * (PRICE_MAX - PRICE_MIN) + offset
        long_liq = max(0.0, seeded_noise(index + model) + 0.55) * 2_800_000
        short_liq = max(0.0, seeded_noise(index + 11 + model) + 0.48) * 2_300_000
        total = long_liq + short_liq
        score = clamp(total / 5_100_000, 0, 1)
        confidence = clamp(0.62 + model * 0.07 + (0.05 if response_range == "90d" else 0), 0, 0.93)
        dominant_side = "long" if long_liq > short_liq * 1.35 else "short" if short_liq > long_liq * 1.35 else "balanced"
        buckets.append(
            HeatmapBucket(
                ...
                relative_intensity=clamp(score * confidence, 0, 1),
                dominant_side=dominant_side,
                estimated_liq_usd=total,
                consumed_score=0,
            )
        )
    return buckets
...
def build_mock_heatmap(
    ...
    candles = build_candles(reference_price)
    ...
    return HeatmapResponse(
        ...
        candles=candles,
        heat_bands=build_heat_bands(normalized_model, reference_price=last_price),
        profile=build_profile(reference_price=last_price),
        net=build_net(candles),
        buckets=build_buckets(normalized_model, response_range, reference_price=last_price),
        exchange_weights=[
